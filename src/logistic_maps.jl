...
maps = Dict(
    ...
    #"m_like" => (x,r) -> 3.8 * r * (.1 + abs(x - 0.5)) * (1 - (2x - 1)^2)
)
...
derivatives = Dict(
    ...
    #"m_like" => (x,r) -> x > 0.5 ? 3.8 * r * (1 * (1 - (2x - 1)^2) - 8 * (2x - 1) * (.1 + x - 0.5)) : 3.8 * r * (-1 * (1 - (2x - 1)^2) - 8 * (2x - 1) * (.1 + 0.5 - x))#x < 0.5 ? 3.8 * r * (1 * (1 - (2x - 1)^2) - 8 * (2x - 1) * (.1 + 0.5 - x)) : 3.8 * r * (1 * (1 - (2x - 1)^2) - 8 * (2x - 1) * (.1 + x - 0.5))#2.99r * ((-cos(π * x) * π) * (1 - (2x - 1)^2) + (1.1 - sin(π * x)) * -8 * (2x - 1))
)
